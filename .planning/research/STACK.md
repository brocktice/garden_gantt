...
**Domain:** Single-user, browser-only garden-planning SPA — interactive (drag-to-adjust) gantt + calendar, ZIP-based frost-date lookup, hybrid plant database (curated + Permapeople API + user-custom), localStorage persistence, static-site hostable, share-worthy polish.
...
**Why Frappe Gantt is the fallback, not the primary:**
...
**Custom build verdict:** Skip it. `@dnd-kit/core 6.3.1` + `interactjs 1.10.27` could build a custom gantt, but the time cost is 1-3 weeks vs. days for SVAR. For a single-user hobby project the share-worthy bar is "looks good and works" — buy that, don't build it.
...
**react-big-calendar caveat:** MIT-licensed, smaller, but requires more elbow-grease for visual polish. If you've never built calendar UI, FullCalendar gets you to share-worthy faster.
...
**Why date-fns v4 over Temporal API:**
...
**Why not Luxon, Day.js, or Moment:**
...
**Critical date discipline (referenced in PITFALLS.md):** ALL stored dates use UTC midday (`12:00Z`). Display converts to local. `last_frost_date` for ZIP 80401 is "May 15" — store as `2026-05-15T12:00:00Z`, never as a local timestamp. This avoids the classic "frost date drifts by a day in some timezones" bug.
...
**Why Zustand over Jotai / Redux Toolkit / Context+useReducer:**
...
```ts
// store.ts
const usePlanStore = create(persist((set, get) => ({
  plantings: [],
  setStartDate: (id, newStart) => set(state => {
    const planting = state.plantings.find(p => p.id === id);
    planting.indoor_start = newStart;
    // Cascade: harden_off, transplant, harvest derive from indoor_start + per-plant constants
    cascadeDownstream(planting); // mutate via immer middleware
  }),
}), { name: 'garden-plan-v1' }));
```
...
**Strategy: ship a static JSON dataset, no runtime API.**
...
**Why static-ship and not phzmapi.org:**
...
**Risk:** ZIP coverage gaps (frostline doesn't have every ZIP). Mitigation: nearest-neighbor lookup at build time using a ZIP centroid file (e.g., `midwire/free_zipcode_data`).

### Plant Data: Permapeople API

**Status verified 2026-04-26:** Permapeople API is up. `/knowledgebase/api-docs/` last updated May 2021. **Manual key approval required** (signup at `/api_requests/new`).
...
**Implication for architecture (matters a lot):**
...
**Fallback plan:**
1. **Primary:** Curated JSON catalog of ~50 plants — author by hand or scrape from gardening references; this powers the core app.
2. **Optional enrichment:** Permapeople for descriptions/images IF user provides API key in settings.
3. **User custom plants:** Inline form, stored in localStorage with the plan.
...
**Test priorities for this app:**
1. **Date math (Vitest unit):** frost-date offsets, succession intervals, indoor-start calculations — pure functions, easy to lock down
2. **Cascade logic (Vitest unit):** drag a bar → recompute downstream → constraints honored (e.g., transplant ≥ last frost)
3. **localStorage roundtrip (Vitest browser):** save plan → reload → identical state
4. **Gantt drag (Vitest browser w/ Playwright):** simulate drag, assert downstream bars moved correctly
5. **E2E happy path (Playwright):** ZIP → zone → add plant → see gantt → drag bar → see cascade → toggle calendar → export JSON → reimport. ~5 tests max.

**Why not Jest:** Vitest is 5-8x faster on Vite projects, native ESM, native TypeScript, and Vitest 4's browser mode gives you Playwright-backed component tests in the same harness as your unit tests. No reason to choose Jest in 2026.
...
# Dates
npm install date-fns@4 @date-fns/utc
...
# Testing
npm install -D vitest @vitest/browser @playwright/test playwright msw
```
...
**If Permapeople API turns out to have CORS blocked on browser requests:**
...
**If user wants offline-first / PWA:**
...
**If user wants multi-device sync later (out of scope for v1):**
...
**Gantt libraries (HIGH confidence on SVAR & Frappe via direct repo verification):**
...
**Permapeople API (verified directly):**
...
1. **Permapeople CORS unverified** — Phase 1 spike: hit the API from a browser; if blocked, drop or proxy. Don't build features on top of it before this is verified.
2. **Permapeople API key approval is manual** — User signup may take days. Design for API to be optional from day one.
3. **frostline ZIP coverage gaps** — Some ZIPs missing. Build-time fallback to nearest centroid lookup.
4. **Date timezone drift** — Strict discipline: store UTC noon, render local. Lock this in via lint rule or TS branded type early.
5. **Tailwind v4 is new** — v4 was a substantial rewrite; minor pre-1.0-engine wrinkles possible. Worst case: pin to last known-good v4 minor.
6. **SVAR React Gantt is younger than Frappe** — 13 commits visible at the time of research; smaller community. If maintenance stalls, switch to Frappe (compatible interaction model). Document the swap path.
7. **localStorage 5MB cap** — Plenty for one season's plan, but warn user before they paste 10,000 task notes. Add an export-and-clear UX.

---

*Stack research for: single-user, browser-only, drag-interactive garden-planning SPA*
*Researched: 2026-04-26*
